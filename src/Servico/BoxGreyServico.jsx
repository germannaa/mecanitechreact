...
export function BoxGreyServico() {
  ...
  }, [modalOpen, modalOpenServico, servicoSelecionado, setServicos]);

  console.log("servicos", servicos);

  const criarNovoServico = () => {
    setModalOpenServico(true);
    setServicoSelecionado(null);
  };

  const handleEditServico = (id_servico) => {
    console.log("EDITAR SERVIÇO");
    const servico = servicos.find((servico) => servico.id_servico === id_servico);
    setServicoSelecionado(servico);
    setModalOpenServico(true);
    console.log("SEEERVICO SELECIONADI", servicoSelecionado)
    console.log("SEEERVICO", servico)

  };

  const handleDeleteServico = (id_servico) => {
    console.log("DELETAR SERVIÇO");
    if (window.confirm("Tem certeza que deseja excluir o serviço?")) {
      axios
        .delete(`http://localhost:3333/servicos/${id_servico}`)
        .then((response) => {
          console.log(response.data);
